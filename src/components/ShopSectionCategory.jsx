...
const ShopSection = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://instagrocerrenderserver.up.railway.app/api/products/c/${id}`)
      .then((response) => {
  ...
  <h2 className="text-3xl font-semibold text-center">Category Page</h2>
</section>
            ...
            <div className={`list-grid-wrapper ${grid ? "list-view" : ""}`}>
              {products && products.length > 0 ? (
                products.map((product) => (
                  <div
                    key={product.id}
                    className="product-card h-100 p-16 border border-gray-200 hover-border-main-600 rounded-16 position-relative transition-2"
                  >
                    <Link
                ...
                <h4 className="text-gray-500 fw-normal center">No Product Found of this category.</h4>
              )}
            </div>
          </div>
          {/* Content End */}
